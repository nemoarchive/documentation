...
```bash
$ ascp -l 1000M -k 2 -QT /home/user/my_data/my_data.tar.gz user123@aspera.nemoarchive.org:v34kltf7/
```
...
```bash
$ ascp -l 1000M -k 2 -QT /home/user/my_data/ user123@aspera.nemoarchive.org:v34kltf7/
```

It is important to note that the above does NOT guarantee that the transfer will proceed at 1000Mbps. This is only a target rate. The NeMO Aspera server is capped to 1000Mbps (1Gbps) in aggregate, so the bandwidth
is shared across all concurrent users. This may cause some contention, resulting in a lower effective transfer speed, depending on the demand.
